...
interface BuyerBlastCampaignManagerProps {
    buyers: Buyer[];
    agents: Agent[];
...
export const BuyerBlastCampaignManager: React.FC<BuyerBlastCampaignManagerProps> = ({ 
    ...
    const listGroups = [
        {
            ...
            items: [
                { id: 'buyer:all', name: 'All Buyers', count: (buyers || []).length },
                { id: 'buyer:New Lead', name: 'New Leads', count: (buyers || []).filter(b => b.status?.includes('New Lead')).length },
                { id: 'buyer:Vetted Buyer', name: 'Vetted Buyers', count: (buyers || []).filter(b => b.status?.includes('Vetted Buyer')).length },
                { id: 'buyer:VIP Buyer', name: 'VIP Buyers', count: (buyers || []).filter(b => b.status?.includes('VIP Buyer')).length },
                { id: 'buyer:Repeat Buyer', name: 'Repeat Buyers', count: (buyers || []).filter(b => b.status?.includes('Repeat Buyer')).length },
            ]
        },
        {
                        ...
                        <div className="flex flex-col space-y-4 animate-in slide-in-from-right-4 duration-300 flex-1 min-h-0">
                            ...
                            <div className="bg-[#1e293b] border border-slate-700 rounded-xl p-3 flex flex-wrap items-center gap-4 shadow-lg">
                                ...
                                <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-1 gap-1">
                                    {(['Buyer', 'Wholesaler', 'Other'] as const).map((type) => (
                                        <button
                                            key={type}
