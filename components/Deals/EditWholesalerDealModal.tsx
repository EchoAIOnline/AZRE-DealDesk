...
export const EditWholesalerDealModal: React.FC<EditWholesalerDealModalProps> = ({ 
    ...
    const handleSendLOI = async () => {
        ...
        if (!targetEmail) {
            setEmailStatus({ type: 'error', message: "This wholesaler does not have an email address set." });
            return;
        }
                 ...
                 <form onSubmit={(e) => { e.preventDefault(); triggerSave(); }} className="p-6 space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div><label className="text-xs text-gray-500 block mb-1 font-bold uppercase">Deal Description</label><textarea className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-3 text-gray-900 dark:text-white text-sm focus:border-blue-500 outline-none h-48 resize-y min-h-[120px]" value={deal.listingDescription || ''} onChange={e => updateDealState({listingDescription: e.target.value})} onBlur={handleAutoSave} placeholder="Paste full property description here..." /></div>
                       </div>
                   </div>
