                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center space-y-3">
                        ...
                        {onGenerateEmail && (
                            <div className="pt-2 flex items-center gap-2">
                                ...
                                {(deal?.dispo?.loiSentAgents?.includes(agent.id) || deal?.loiSent && (slotIndex === 1 || (deal?.agentName && agent?.name && (deal.agentName || '').toLowerCase() === (agent.name || '').toLowerCase()))) && (
...
export const EditDealModal: React.FC<EditDealModalProps> = ({ 
    ...
    const saveCurrentDeal = useCallback(async () => {
        ...
        try {
            ...
            if (savedDeal && savedDeal.id) {
                ...
                if (savedDeal.createdAt && dealRef.current.createdAt !== savedDeal.createdAt) {
                    dealRef.current.createdAt = savedDeal.createdAt;
                    changed = true;
                }
                if (changed) {
                    setDeal(prev => ({ ...prev, id: savedDeal.id, createdAt: savedDeal.createdAt }));
                }
            }
    ...
    triggerSave(); 
                   ...
                   <div className="space-y-4">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700/50">
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-3 pl-8 text-gray-900 dark:text-white text-xl font-bold focus:border-blue-500 outline-none" value={formatNumberWithCommas(deal.listPrice) || ''} onChange={e => {
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-2 pl-6 text-gray-900 dark:text-white text-sm focus:border-blue-500 outline-none" value={formatNumberWithCommas(deal.originalAskingPrice) || ''} onChange={e => {
                                                        const val = parseNumberFromCurrency(e.target.value);
                                        ...
                                        <div>
                                            ...
                                            <select className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-2 text-gray-900 dark:text-white text-sm focus:border-blue-500 outline-none" value={deal.priceReductionAlert || ''} onChange={e => { updateDealState({priceReductionAlert: e.target.value}); if(onUpdate) onUpdate(deal.id, {priceReductionAlert: e.target.value}); triggerSave(); }}><option value="">None</option><option value="Asking Price Has Been Reduced">Asking Price Has Been Reduced</option><option value="Price Reduced Multiple Times">Price Reduced Multiple Times</option></select>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div><label className="text-xs text-green-600 dark:text-green-500 block mb-1 font-bold">My Cash Offer</label><div className="relative"><span className="absolute left-3 top-3 text-green-600 dark:text-green-500 text-lg font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-green-200 dark:border-green-800/50 rounded p-3 pl-8 text-green-600 dark:text-green-400 text-xl font-bold focus:border-blue-500 outline-none" value={formatNumberWithCommas(deal.offerPrice) || ''} onChange={e => updateDealState({offerPrice: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} placeholder="Enter Amount" /></div></div>
                                    <div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-green-600 dark:text-green-500 block mb-1 font-bold">Accepted Asking Price</label><div className="relative"><span className="absolute left-2 top-2 text-green-600 dark:text-green-500">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-2 pl-6 text-green-600 dark:text-green-400 text-sm font-bold focus:border-blue-500 outline-none" value={formatNumberWithCommas(deal.negotiatedAskingPrice) || ''} onChange={e => updateDealState({negotiatedAskingPrice: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} /></div></div><div><label className="text-xs text-blue-600 dark:text-blue-400 block mb-1 font-bold">Estimated Wholesale Profit</label><div className="relative"><span className="absolute left-2 top-2 text-blue-600 dark:text-blue-500 font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-blue-200 dark:border-blue-900/50 rounded p-2 pl-6 text-blue-600 dark:text-blue-400 font-bold text-sm focus:border-blue-500 outline-none" value={formatNumberWithCommas(deal.desiredWholesaleProfit) || ''} onChange={e => updateDealState({desiredWholesaleProfit: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} /></div></div></div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4"><div><label className="text-xs text-gray-500 block mb-1 font-bold">Renovation ARV</label><div className="relative"><span className="absolute left-4 top-4 text-green-600 dark:text-green-500 text-xl font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-4 pl-10 text-green-600 dark:text-green-400 placeholder-green-600 dark:placeholder-green-400 text-2xl font-bold focus:border-green-500 outline-none transition-colors" value={formatNumberWithCommas(deal.renovationARV) || ''} onChange={e => updateDealState({renovationARV: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} placeholder="0" /></div></div><div><label className="text-xs text-gray-500 block mb-1 font-bold">Renovation Estimate</label><div className="relative"><span className="absolute left-4 top-4 text-gray-400 dark:text-gray-500 text-xl font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-4 pl-10 text-gray-900 dark:text-white placeholder-gray-900 dark:placeholder-white text-2xl font-bold focus:border-blue-500 outline-none transition-colors" value={formatNumberWithCommas(deal.renovationEstimate) || ''} onChange={e => updateDealState({renovationEstimate: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} placeholder="0" /></div></div></div>
                                <div className="grid grid-cols-2 gap-4 mb-4"><div><label className="text-xs text-gray-500 block mb-1 font-bold">New Construction ARV</label><div className="relative"><span className="absolute left-4 top-4 text-green-600 dark:text-green-500 text-xl font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-4 pl-10 text-green-600 dark:text-green-400 placeholder-green-600 dark:placeholder-green-400 text-2xl font-bold focus:border-green-500 outline-none transition-colors" value={formatNumberWithCommas(deal.newConstructionARV) || ''} onChange={e => updateDealState({newConstructionARV: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} placeholder="0" /></div></div><div><label className="text-xs text-gray-500 block mb-1 font-bold">New Construction Estimate</label><div className="relative"><span className="absolute left-4 top-4 text-gray-400 dark:text-gray-500 text-xl font-bold">$</span><input type="text" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-4 pl-10 text-gray-900 dark:text-white placeholder-gray-900 dark:placeholder-white text-2xl font-bold focus:border-blue-500 outline-none transition-colors" value={formatNumberWithCommas(deal.newConstructionEstimate) || ''} onChange={e => updateDealState({newConstructionEstimate: parseNumberFromCurrency(e.target.value)})} onBlur={handleAutoSave} placeholder="0" /></div></div></div>
                   ...
                   <div className="space-y-4">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-800 rounded border border-gray-300 dark:border-gray-700 p-4 h-48 overflow-y-auto space-y-2">
                                    ...
                                    <div key={idx} className="flex flex-col gap-2 border-b border-gray-100 dark:border-gray-800/50 pb-2 last:border-0 last:pb-0 group relative">
                                        {editingLogIndex === idx ? (
                                            <div className="flex flex-col gap-2 w-full">
                                                <textarea
                                                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-2 text-sm focus:border-blue-500 outline-none resize-none"
                                                    value={editLogValue}
                        ...
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded p-3 text-gray-900 dark:text-white text-sm focus:border-blue-500 outline-none" 
                                value={tempLogValue} 
